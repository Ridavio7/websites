...
/*jshint -W083 */
	...
	let instance_counter = 0,
		...
		document = window.document; // local letiable is always faster to access then a global
	...
	/**
	 * holds all jstree related functions and letiables, including the actual class and methods to create, access and manipulate instances.
	 * @name $.jstree
	 */
	$.jstree = {
		/**
	...
	$.fn.jstree = function (arg) {
		...
		let is_method	= (typeof arg === 'string'),
	...
	$.jstree.defaults.core = {
		...
		data			: false,
		/**
		 * configure the letious strings used throughout the tree
		 *
		...
		themes			: {
			...
			stripes			: false,
			/**
			 * a string (or boolean `false`) specifying the theme letiant to use (if the theme supports letiants)
			 * @name $.jstree.defaults.core.themes.letiant
			 */
			letiant			: false,
			/**
		...
		keyboard : {
			...
			'left': function (e) {
				...
				if(this.is_open(e.currentTarget)) {
					...
					let o = this.get_parent(e.currentTarget);
			...
			'up': function (e) {
				// up
				e.preventDefault();
				let o = this.get_prev_dom(e.currentTarget);
			...
			'right': function (e) {
				...
				if(this.is_closed(e.currentTarget)) {
					...
					let o = this.get_node(e.currentTarget, true).children('.jstree-children')[0];
			...
			'down': function (e) {
				// down
				e.preventDefault();
				let o = this.get_next_dom(e.currentTarget);
				if(o && o.length) { o.children('.jstree-anchor').trigger('focus'); }
			},
			'*': function (e) {
			...
			'home': function (e) {
				// home
				e.preventDefault();
				let o = this._firstChild(this.get_container_ul()[0]);
	...
	$.jstree.core.prototype = {
		...
		_create_prototype_node : function () {
			let _node = document.createElement('LI'), _temp1, _temp2;
		...
		_kbevent_to_func : function (e) {
			...
			let kb = this.settings.core.keyboard, i, tmp;
			for (i in kb) {
		...
		bind : function () {
			...
			this.element
				...
				.on('keypress.jstree', function (e) {
						...
						let chr = String.fromCharCode(e.which).toLowerCase(),
				...
				.on("init.jstree", function () {
						let s = this.settings.core.themes;
						this._data.core.themes.dots			= s.dots;
						this._data.core.themes.stripes		= s.stripes;
						this._data.core.themes.icons		= s.icons;
						this._data.core.themes.ellipsis		= s.ellipsis;
						this.set_theme(s.name || "default", s.url);
						this.set_theme_letiant(s.letiant);
					}.bind(this))
				...
				.on('focus.jstree', '.jstree-anchor', function (e) {
						let tmp = this.get_node(e.currentTarget);
				...
				.on('focus.jstree', function () {
						if(+(new Date()) - was_click > 500 && !this._data.core.focused && this.settings.core.restore_focus) {
							was_click = 0;
							let act = this.get_node(this.element.attr('aria-activedescendant'), true);
							if(act) {
		...
		get_string : function (key) {
			let a = this.settings.core.strings;
		...
		load_node : function (obj, callback) {
			let dom = this.get_node(obj, true), k, l, i, j, c;
			...
			this._load_node(obj, function (status) {
				...
				let dom = this.get_node(obj, true), i = 0, j = 0, m = this._model.data, has_children = false;
		...
		_load_node : function (obj, callback) {
			let s = this.settings.core.data, t;
			let notTextOrCommentNode = function notTextOrCommentNode () {
      ...
      if (obj && $.inArray(obj.id, this._model.changed) === -1) {
		...
		_append_html_data : function (dom, data, cb) {
			...
			dom.children = [];
			dom.children_d = [];
			let dat = data.is('ul') ? data.children() : data,
				par = dom.id,
				chd = [],
				dpc = [],
				m = this._model.data,
		...
		_append_json_data : function (dom, data, cb, force_processing) {
			...
			let w = null,
				...
				rslt = function (rslt, worker) {
					...
					if(worker) {
						let j, a = rslt.add, r = rslt.sel, s = this._data.core.selected.slice();
						m = this._model.data;
		...
		_parse_model_from_json : function (d, p, ps) {
			...
			if(p) { ps.unshift(p); }
			let tid = false, i, j, c, e, m = this._model.data, df = this._model.default_state, tmp;
			do {
				tid = 'j' + this._id + '_' + (++this._cnt);
			} while(m[tid]);
		 ...
		 */
		_redraw : function () {
			let nodes = this._model.force_full_redraw ? this._model.data[$.jstree.root].children.concat([]) : this._model.changed.concat([]),
		...
		open_all : function (obj, animation, original_obj) {
			...
			obj = this.get_node(obj);
			if(!obj) { return false; }
			let dom = obj.id === $.jstree.root ? this.get_container_ul() : this.get_node(obj, true), i, j, _this;
			if(!dom.length) {
		...
		hover_node : function (obj) {
			...
			let o = this.element.find('.jstree-hovered'), t = this.element;
		...
		select_all : function (supress_event) {
			let tmp = this._data.core.selected.concat([]), i, j;
		...
		refresh : function (skip_loading, forget_state) {
			...
			let c = this.get_container_ul()[0].className;
			if(!skip_loading) {
		...
		refresh_node : function (obj) {
			...
			let opened = [], to_load = [], s = this._data.core.selected.concat([]);
			to_load.push(obj.id);
		...
		set_id : function (obj, id) {
			obj = this.get_node(obj);
			if(!obj || obj.id === $.jstree.root) { return false; }
			let i, j, m = this._model.data, old = obj.id;
			id = id.toString();
		 ...
		 * @param  {mixed} more some letious additional information, for example if a "move_node" operations is triggered by DND this will be the hovered node
		 * @return {Boolean}
		 */
		...
		edit : function (obj, default_text, callback) {
			let rtl, w, a, s, t, h1, h2, fn, tmp, cancel = false;
			obj = this.get_node(obj);
			if(!obj) { return false; }
			...
			h2 = $("<"+"input />", {
						...
						"on" : {
							"blur" : function (e) {
								...
								e.preventDefault();
								let i = s.children(".jstree-rename-input"),
									v = i.val(),
								...
								this._data.core.focused = tmp.id;
								setTimeout(function () {
									let node = this.get_node(tmp.id, true);
									if(node.length) {
										this._data.core.focused = tmp.id;
		...
		/**
		 * changes the theme letiant (if the theme has letiants)
		 * @name set_theme_letiant(letiant_name)
		 * @param {String|Boolean} letiant_name the letiant to apply (if `false` is used the current letiant is removed)
		 */
		set_theme_letiant : function (letiant_name) {
			if(this._data.core.themes.letiant) {
				this.element.removeClass('jstree-' + this._data.core.themes.name + '-' + this._data.core.themes.letiant);
			}
			this._data.core.themes.letiant = letiant_name;
			if(letiant_name) {
				this.element.addClass('jstree-' + this._data.core.themes.name + '-' + this._data.core.themes.letiant);
			}
		},
		/**
		 * gets the name of the currently applied theme letiant
		 * @name get_theme()
		 * @return {String}
		 */
		get_theme_letiant : function () { return this._data.core.themes.letiant; },
		/**
 ...
 */
	...
	$.jstree.plugins.checkbox = function (options, parent) {
				...
				this.element
					...
					.on(this.settings.checkbox.tie_selection ? 'select_node.jstree' : 'check_node.jstree', function (e, data) {
							...
							if(s.indexOf('down') !== -1) {
								...
								let selectedIds = this._cascade_new_checked_state(obj.id, true);
								let temp = obj.children_d.concat(obj.id);
					...
					.on(this.settings.checkbox.tie_selection ? 'deselect_node.jstree' : 'uncheck_node.jstree', function (e, data) {
							...
							// apply down
							if(s.indexOf('down') !== -1) {
								let selectedIds = this._cascade_new_checked_state(obj.id, false);
		...
		this.get_undetermined = function (full) {
			...
			let i, j, k, l, o = {}, m = this._model.data, t = this.settings.checkbox.tie_selection, s = this._data[ t ? 'core' : 'checkbox' ].selected, p = [], tt = this, r = [];
			for(i = 0, j = s.length; i < j; i++) {
		...
		this._undetermined = function () {
			if(this.element === null) { return; }
			let p = this.get_undetermined(false), i, j, s;
		...
		this.is_undetermined = function (obj) {
			obj = this.get_node(obj);
			let s = this.settings.checkbox.cascade, i, j, t = this.settings.checkbox.tie_selection, d = this._data[ t ? 'core' : 'checkbox' ].selected, m = this._model.data;
		...
		this._cascade_new_checked_state = function (id, checkedState) {
			let self = this;
			let t = this.settings.checkbox.tie_selection;
			let node = this._model.data[id];
			let selectedNodeIds = [];
			let selectedChildrenIds = [], i, j, selectedChildIds;
			...
			if (
				...
				let dom = self.get_node(node, true);

				//A node's state is undetermined if some but not all of it's children are checked/selected .
				let undetermined = selectedChildrenIds.length > 0 && selectedChildrenIds.length < node.children.length;
		...
		this.get_checked_descendants = function (id) {
			let self = this;
			let t = self.settings.checkbox.tie_selection;
			let node = self._model.data[id];
		...
		this.check_all = function () {
			...
			let tmp = this._data.checkbox.selected.concat([]), i, j;
		...
		this.get_state = function () {
			let state = parent.get_state.apply(this, arguments);
			if(this.settings.checkbox.tie_selection) { return state; }
			state.checkbox = this._data.checkbox.selected.slice();
			return state;
		};
		this.set_state = function (state, callback) {
			let res = parent.set_state.apply(this, arguments);
 ...
 */
			...
			return {
				"create" : {
					...
					"action"			: function (data) {
						...
						let obj_id = document.getElementById(obj.a_attr.id);

						let li = document.createElement('li'); li.className = "jstree-node  jstree-leaf"; li.role = "none";
						let i_branch = document.createElement('i'); i_branch.className = "jstree-icon jstree-ocl"; i_branch.role = "presentation";
						let input = document.createElement('input'); input.className = "jstree-anchor jstree-clicked"; input.id = "new_folder"; input.value = "Имя папки...";
						li.append(i_branch);
						li.append(input);

						console.log(obj_id.nextSibling);
						if(obj_id.nextSibling === null){
							obj_id.after(li);
						} else {
							obj_id.nextSibling.prepend(li);
						}

						input = document.getElementById("new_folder");
						$(input).keyup(function(event) {
							if (event.keyCode === 13) {
								$("#sub_create_node").click();
							}
						});

						$("#sub_create_node").click(function() {
							let body  =  {"user":"demo", "meth":"add", "obj":"dirC", "uinparent":"", "name":""};
							body.uinparent = obj.id;
				...
				"rename" : {
					...
					"action"			: function (data) {
						...
						let teg = document.getElementById(obj.a_attr.id);
						let input = document.createElement('input'); input.className = "jstree-anchor jstree-clicked"; input.id = `${obj.a_attr.id}`; input.value = `${obj.text}`;
						teg.before(input);
						teg.remove();

						input = document.getElementById(obj.a_attr.id);
						$(input).keyup(function(event) {
							if (event.keyCode === 13) {
								$("#sub_rename_node").click();
								
								let a = document.createElement('a'); a.className = "jstree-anchor jstree-clicked"; a.id = `${obj.a_attr.id}`; a.href=`${obj.a_attr.href}`;
								let i = document.createElement('i'); i.className = "jstree-icon jstree-themeicon"; i.role = "presentation";
								a.append(i);
								a.append(`${input.value}`);
								input.before(a);
								input.remove();
							}
						});

						$("#sub_rename_node").click(function() {
							let body  =  {"user":"demo", "meth":"update", "obj":"dirC", "uin":"", "uinparent":"", "name":"" };
							body.uin = obj.id;
							body.uinparent = obj.parent;
							body.name = input.value;
							funcCommand(body, funcProcessOnlyInfo);
						});
					}
				...
				"remove" : {
					...
					"label"				: function (data){
						...
						let obj = inst.get_node(data.reference);
						let lable;
						obj.original.del === 1 ? lable = "Восстановить" : lable = "Удалить";
					...
					"action"			: function (data){
						...
						let body  =  {"user":"demo", "meth":"mdel", "obj":"catC", "uin":""};
						body.uin = obj.id;
						funcCommand(body, funcProcessOnlyInfo);
					}
				},
	...
	$.jstree.plugins.contextmenu = function (options, parent) {
		...
		this.show_contextmenu = function (obj, x, y, e) {
			obj = this.get_node(obj);
			if(!obj || obj.id === $.jstree.root) { return false; }
			let s = this.settings.contextmenu,
				d = this.get_node(obj, true),
		...
		this._show_contextmenu = function (obj, x, y, i) {
			let d = this.get_node(obj, true),
				a = d.children(".jstree-anchor");
			$(document).one("context_show.vakata.jstree", function (e, data) {
				let cls = 'jstree-contextmenu jstree-' + this.get_theme() + '-contextmenu';
	...
	(function ($) {
		...
		$.vakata.context = {
			...
			show : function (reference, position, data) {
				let o, e, x, y, w, h, dw, dh, cond = true;
		...
		$(function () {
			...
			vakata_context.element
				...
				.on('keydown', function (e) {
					e.preventDefault();
					let a = vakata_context.element.find('.vakata-contextmenu-shortcut-' + e.which).parent();
		...
		this.bind = function () {
			...
			this.element
				.on(this.settings.dnd.use_html5 ? 'dragstart.jstree' : 'mousedown.jstree touchstart.jstree', this.settings.dnd.large_drag_target ? '.jstree-node' : '.jstree-anchor', function (e) {
						...
						let obj = this.get_node(e.target),
						...
						if(obj && (obj.id || obj.id === 0) && obj.id !== $.jstree.root && (e.which === 1 || e.type === "touchstart" || e.type === "dragstart") &&
							...
							if (this.settings.dnd.use_html5) {
								...
								return $.vakata.dnd.start(e, drg, '<div id="jstree-dnd" class="jstree-' + this.get_theme() + ' jstree-' + this.get_theme() + '-' + this.get_theme_letiant() + ' ' + ( this.settings.core.themes.responsive ? ' jstree-dnd-responsive' : '' ) + '"><i class="jstree-icon jstree-er"></i>' + txt + '<ins class="jstree-copy">+</ins></div>');
							}
						}
					}.bind(this));
			if (this.settings.dnd.use_html5) {
	...
	$(function() {
		...
		$(document)
			...
			.on('dnd_move.vakata.jstree', function (e, data) {
				let isDifferentNode = data.event.target !== lastev.target;
				if(opento) {
				...
				let ins = $.jstree.reference(data.event.target),
					ref = false,
					off = false,
					rel = false,
				...
				if(ins && ins._data && ins._data.dnd) {
					...
					data.helper
						.children().attr('class', 'jstree-' + ins.get_theme() + ' jstree-' + ins.get_theme() + '-' + ins.get_theme_letiant() + ' ' + ( ins.settings.core.themes.responsive ? ' jstree-dnd-responsive' : '' ))
	...
	(function ($) {
		...
		$.vakata.dnd = {
			...
			_scroll : function (init_only) {
				...
				let i = vakata_dnd.scroll_e.scrollTop(),
			...
			stop : function (e) {
				...
				if(vakata_dnd.is_drag) {
					...
					if(e.type === "touchend" && e.target === vakata_dnd.target) {
						let to = setTimeout(function () { $(e.target).trigger('click'); }, 100);
 ...
 */
	...
	$.jstree.plugins.massload = function (options, parent) {
		...
		this._load_node = function (obj, callback) {
			let data = this._data.massload[obj.id],
				rslt = null, dom;
			if(data) {
 ...
 */
			...
			this.element
				.on("search.jstree", function (e, data) {
						if(this._data.search.som && data.res.length) {
							let m = this._model.data, i, j, p = [], k, l;
	...
	(function ($) {
		...
		$.vakata.search = function(pattern, txt, options) {
			...
			let MATCH_LOCATION	= options.location,
 ...
 */
	...
	$.jstree.plugins.state = function (options, parent) {
		...
		this.save_state = function () {
			let tm = this.get_state();
			if (!this.settings.state.preserve_loaded) {
				delete tm.core.loaded;
			}
			let st = { 'state' : tm, 'ttl' : this.settings.state.ttl, 'sec' : +(new Date()) };
 ...
 */
	...
	$.jstree.plugins.types = function (options, parent) {
		...
		this.check = function (chk, obj, par, pos, more) {
			...
			let m = obj && (obj.id || obj.id === 0) ? (more && more.origin ? more.origin : $.jstree.reference(obj.id)) : null, tmp, d, i, j;
		...
		this.get_rules = function (obj) {
			obj = this.get_node(obj);
			if(!obj) { return false; }
			let tmp = this.get_type(obj, true);
		...
		this.set_type = function (obj, type) {
			let m = this._model.data, t, t1, t2, old_type, old_icon, k, d, a;
			if($.vakata.is_array(obj)) {
				obj = obj.slice();
				for(t1 = 0, t2 = obj.length; t1 < t2; t1++) {
	...
	$.jstree.plugins.unique = function (options, parent) {
		...
		this.create_node = function (par, node, pos, callback, is_loaded) {
			if(!node || (typeof node === 'object' && node.text === undefined)) {
				...
				if(!node) { node = {}; }
				let tmp, n, dpc, i, j, m = this._model.data, s = this.settings.unique.case_sensitive, w = this.settings.unique.trim_whitespace, cb = this.settings.unique.duplicate, t;
			...
			this.element
				...
				.on("contextmenu.jstree", ".jstree-wholerow", function (e) {
						if (this._data.contextmenu) {
							e.preventDefault();
							let tmp = $.Event('contextmenu', { metaKey : e.metaKey, ctrlKey : e.ctrlKey, altKey : e.altKey, shiftKey : e.shiftKey, pageX : e.pageX, pageY : e.pageY });
				...
				.on("mousedown.jstree touchstart.jstree", ".jstree-wholerow", function (e) {
						if(e.target === e.currentTarget) {
							let a = $(e.currentTarget).closest(".jstree-node").children(".jstree-anchor");
							e.target = a[0];
							a.trigger(e);
						}
					})
				*/
				.on("click.jstree", ".jstree-wholerow", function (e) {
						e.stopImmediatePropagation();
						let tmp = $.Event('click', { metaKey : e.metaKey, ctrlKey : e.ctrlKey, altKey : e.altKey, shiftKey : e.shiftKey });
						$(e.currentTarget).closest(".jstree-node").children(".jstree-anchor").first().trigger(tmp).trigger('focus');
					})
				.on("dblclick.jstree", ".jstree-wholerow", function (e) {
						e.stopImmediatePropagation();
						let tmp = $.Event('dblclick', { metaKey : e.metaKey, ctrlKey : e.ctrlKey, altKey : e.altKey, shiftKey : e.shiftKey });
						$(e.currentTarget).closest(".jstree-node").children(".jstree-anchor").first().trigger(tmp).trigger('focus');
					})
