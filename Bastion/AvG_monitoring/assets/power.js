...
function callbackGetFiltObjectList(err, respobj){
    if(err){
        ...
        if (chose_box == "Фильтр"){
            funcGetObjectList();
        } else if(chose_box == "Питание в норме"){
            funcFiltList(respobj, "Питание в норме");
        } else if(chose_box == "Неисправность 220 В") {
            funcFiltList(respobj, "Неисправность 220 В");
        } else if(chose_box == "Нет 220 В и разряжен АКБ") {
            funcFiltList(respobj, "Нет 220 вольт и разаряжен аккумулятор");
        } else if(chose_box == "Разряд АКБ") {
            funcFiltList(respobj, "Разряд АКБ");
        }
    }
